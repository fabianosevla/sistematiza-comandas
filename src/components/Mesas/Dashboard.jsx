  ...
  return (
    <div className="page-content">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '28px' }}>
        <div style={{ background: 'var(--mesa-ocupada-bg)', border: '1px solid var(--mesa-ocupada-bg)', borderRadius: 'var(--radius-lg)', padding: '20px' }}>
          <p style={{ fontSize: '28px', fontWeight: '800', color: 'var(--mesa-ocupada)', letterSpacing: '-1px', lineHeight: 1 }}>{abertas}</p>
          <p style={{ fontSize: '11px', fontWeight: '600', color: 'var(--mesa-ocupada)', marginTop: '6px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Comandas abertas</p>
        </div>
        <div style={{ background: 'var(--mesa-livre-bg)', border: '1px solid var(--mesa-livre-bg)', borderRadius: 'var(--radius-lg)', padding: '20px' }}>
          <p style={{ fontSize: '28px', fontWeight: '800', color: 'var(--mesa-livre)', letterSpacing: '-1px', lineHeight: 1 }}>{fechadas}</p>
          <p style={{ fontSize: '11px', fontWeight: '600', color: 'var(--mesa-livre)', marginTop: '6px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Fechadas hoje</p>
        </div>
        <div style={{ background: 'var(--bg-layer-01)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', padding: '20px' }}>
          <p style={{ fontSize: '22px', fontWeight: '800', color: 'var(--text-primary)', letterSpacing: '-0.5px', lineHeight: 1 }}>{fmt(faturado)}</p>
          <p style={{ fontSize: '11px', fontWeight: '600', color: 'var(--text-secondary)', marginTop: '6px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Faturado hoje</p>
        </div>
      </div>
          ...
          {mesas.map(mesa => {
            ...
            return (
              <div key={mesa.id}
                ...
                style={{ background: bg, border: `1.5px solid ${bord}`, borderRadius: 'var(--radius-xl)', padding: '20px', cursor: 'pointer', transition: 'box-shadow 0.15s, transform 0.1s', position: 'relative' }}
                onMouseEnter={e => { e.currentTarget.style.boxShadow = 'var(--shadow-md)'; e.currentTarget.style.transform = 'translateY(-2px)' }}
                onMouseLeave={e => { e.currentTarget.style.boxShadow = 'none'; e.currentTarget.style.transform = 'none' }}>

                <button
                  onClick={e => { e.stopPropagation(); onRemoverMesa(mesa) }}
                  style={{ position: 'absolute', top: '10px', right: '10px', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: '14px', opacity: 0.35, padding: '2px', transition: 'opacity 0.15s' }}
                  onMouseEnter={e => e.currentTarget.style.opacity = '1'}
                  onMouseLeave={e => e.currentTarget.style.opacity = '0.35'}>
                  ✕
                </button>

                <p style={{ fontSize: '10px', fontWeight: '700', color: 'var(--text-placeholder)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '6px' }}>Mesa</p>
                <p style={{ fontSize: '40px', fontWeight: '800', color: cor, letterSpacing: '-1.5px', lineHeight: 1, marginBottom: '12px' }}>{mesa.numero}</p>

                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: isLivre ? 0 : '10px' }}>
                  <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: cor }} />
                  <span style={{ fontSize: '12px', fontWeight: '600', color: cor }}>{isLivre ? 'Livre' : 'Ocupada'}</span>
                </div>

                {!isLivre && (
                  <div style={{ borderTop: '1px solid rgba(0,0,0,0.06)', paddingTop: '10px' }}>
                    <p style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{qtdComandas} comanda{qtdComandas !== 1 ? 's' : ''}</p>
                    <p style={{ fontSize: '15px', fontWeight: '700', color: 'var(--text-primary)', marginTop: '2px' }}>{fmt(totalMesa)}</p>
                  </div>
                )}
              </div>
            )
